	...
	switch (uMsg)														// Evaluate Window Message
	{
		...
		case WM_KEYDOWN:												// Update Keyboard Buffers For Keys Pressed
			if ((wParam >= 0) && (wParam <= 255))						// Is Key (wParam) In A Valid Range?
			{
				...
				window->keys->keyUp [wParam] = FALSE;					// Set The Selected Key (wParam) To False
				return 0;												// Return
			}
		break;															// Break
		...
		case WM_KEYUP:													// Update Keyboard Buffers For Keys Released
			if ((wParam >= 0) && (wParam <= 255))						// Is Key (wParam) In A Valid Range?
			{
				...
				window->keys->keyUp [wParam] = TRUE;					// Set The Selected Key (wParam) To True
				return 0;												// Return
			}
		break;															// Break
...
int WINAPI WinMain (HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	window.init.title			= appTitle;								// Window Title
	window.init.width			= screenInfo[0];						// Window Width
	window.init.height			= screenInfo[1];						// Window Height
	window.init.bitsPerPixel	= screenInfo[2];						// Bits Per Pixel
	...
	while (g_isProgramLooping)											// Loop Until WM_QUIT Is Received
	{
		...
		if (CreateWindowGL (&window) == TRUE)							// Was Window Creation Successful?
		{
			...
			if (Initialize (&window, &keys, &camera) == FALSE)					// Call User Intialization
			{
				// Failure
